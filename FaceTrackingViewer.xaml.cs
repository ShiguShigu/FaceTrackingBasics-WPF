    ...
    public partial class FaceTrackingViewer : UserControl, IDisposable
    {
        ...
        protected override void OnRender(DrawingContext drawingContext)
        {
            ...
            foreach (SkeletonFaceTracker faceInformation in this.trackedSkeletons.Values)
            {
                ...
                faceInformation.RenderCursorPos(drawingContext);
            }
        ...
        public void ResetAllFaceRotations()
        {
            foreach (SkeletonFaceTracker faceTracker in this.trackedSkeletons.Values)
            {
                faceTracker.CenterRotation();
            }
        }
        ...
        private class SkeletonFaceTracker : IDisposable
        {
            ...
            private Vector3DF initRotation;
            ...
            public void RenderCursorPos(DrawingContext drawingContext)
            {
                double pitch = this.initRotation.X - this.rotation.X; // Pitch
                double yaw = this.initRotation.Y - this.rotation.Y; // Yaw
                double roll = this.initRotation.Z - this.rotation.Z; // Roll


                double x = this.translation.X;
                double y = this.translation.Y;
                double z = this.translation.Z;

                // Console.WriteLine("{0} {1} {2}", x, y, z);
                // Console.WriteLine("{0} {1} {2}", pitch, yaw, roll);
                ...
                double rawYaw = Math.Sin(yaw / 180 * Math.PI);
                double rawPitch = Math.Sin(pitch / 180 * Math.PI);

                double adaptiveYaw = rawYaw;
                double adaptivePitch = rawPitch;

                double mouseX = scale * z * adaptiveYaw + centerX;
                double mouseY = scale * z * rawPitch + centerY;

                drawingContext.DrawEllipse(Brushes.Red, new Pen(Brushes.Blue, 1), new Point(mouseX, mouseY), 30, 30);
            }
